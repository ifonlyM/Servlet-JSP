...
<title>MainStream!-갤러리-${board.title}</title>
...
<main class="detail-main">
        <div class="top"></div>
        <div class="titleBox">
            <p class="title"><b>${board.title}</b></p>
	        ...
	        <div class="attachs">
		       		...
		       		<c:forEach items="${board.attachs }" var="attach">
						<p title="${attach.origin }">
							<a href="${pageContext.request.contextPath }/download?filename=${attach.uuid}">${ attach.origin }</a>
						</p>
		       		</c:forEach>
	        </div>
       	</c:if>
	...
	$(function() {
		...
		$(".modify").find("button").click(function(e){
			...
			if(originWriter !== "${member.id}"){
				...
				location.href = cp + "/board/modify?bno=${board.bno}&category=2";
			}
		});
